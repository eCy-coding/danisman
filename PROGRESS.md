...
## HATASIZ-TESLİM (2026-06-12) — pre-existing CI kırmızıları kök-neden turu
**T0 talimatı:** "tüm hataları gider, hatasız teslim et" → PR #226'daki 4 kırmızı gate'in tamamı (hiçbiri Research Bridge kaynaklı değil; stash-baseline kanıtlı). Kökler: (1) FounderPage 21 test — FAZ-2B `ready` gate'i sonrası iki test dosyasının useTranslation mock'unda `ready` alanı yok; (2) a11y img-alt — scanner JSX yorumlarındaki "<img>" kelimesini markup sanıyor (Hero.tsx:520-521 yorumları, gerçek img alt'lı); (3) canonical — impl S13-R4'te x-default'u apex'e taşıdı, test eski /tr beklentisinde; (4) npm audit — devDep concurrently@9 → shell-quote CRITICAL (GHSA-w7jw-789q-3m8p), fix=concurrently 10; (5) Lighthouse — lighthouserc.json'daki `_comment_thresholds` key'i LHCI'da audit sanılıp `auditRan` fail üretiyor + a11y minScore 1.0 repo'nun diğer iki config'iyle (0.95) tutarsız; (6) migration gate — watermark TARİHSEL migration'ları tarıyor, şemaya dokunan her PR eski Class-C'lerden kırmızı; fix=PR-diff kapsamı (pattern'ler/D-hard-fail aynen) + `labeled` trigger. Allowlist ekleri yukarıdaki kökler için (test dosyaları, Hero, lock, analyzer script).
