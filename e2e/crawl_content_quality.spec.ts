...
async function auditContent(page: Page, pagePath: string): Promise<ContentAudit> {
  ...
  const result = await page.evaluate((baseUrl: string) => {
    ...
    const mainEl = (document.querySelector('main, [role="main"], article, .content') ?? document.body) as HTMLElement;
    const allText = mainEl.innerText ?? mainEl.textContent ?? '';
    const wordCount = allText.trim().split(/\s+/).filter((w: string) => w.length > 2).length;

    // Linkler
    const allLinks = Array.from(document.querySelectorAll('a[href]'));
    const internalLinks = allLinks.filter(a => {
      const href = (a as HTMLAnchorElement).href;
      return href.includes(baseUrl) || (a as HTMLAnchorElement).getAttribute('href')?.startsWith('/');
    }).length;
    ...
    const hHierarchyOk = !(h3s.length > 0 && h2s.length === 0) && !(h2s.length > 0 && h1s.length === 0);
...
test.describe('Crowler: Content Quality & Keyword Audit (P32-T13/T14/T18)', () => {
  ...
  for (const pagePath of keywordPages) {
    test(`Keyword audit: ${pagePath}`, async ({ page }) => {
      ...
      // Çift H1 yok (T13)
      expect(audit.h1Count, `[${pagePath}] Birden fazla H1 var (${audit.h1Count})`).toBeLessThanOrEqual(1);
    });
  }
  ...
  test('İçerik uzunluğu: kritik sayfalar min 150 kelime', async ({ page }) => {
    ...
    const minWords: Record<string, number> = {
      '/': 150, '/services': 150, '/about': 100, '/methodology': 150,
      '/pricing': 80, '/contact': 50,
    };
  ...
  test('T18: Tüm img alt text — kritik sayfalar', async ({ page }) => {
    ...
    await setupMocks(page);
    const criticalPages = ['/', '/services', '/about', '/case-studies', '/blog'];
    const failures: string[] = [];
  ...
  test('T19: URL trailing slash tutarlılığı', async ({ request }) => {
    test.setTimeout(30000);
    const paths = ['/services', '/about', '/blog', '/pricing', '/contact'];
    const issues: string[] = [];

    for (const p of paths) {
      // Trailing slash ile ve slash'sız — her ikisi 200 veya consistent redirect vermeli
      const withSlash = await request.get(`${BASE_URL}${p}/`, { maxRedirects: 3 }).catch(() => null);
      const withoutSlash = await request.get(`${BASE_URL}${p}`, { maxRedirects: 3 }).catch(() => null);
      if (withSlash && withoutSlash) {
        ...
        if (withSlash.status() === 200 && withoutSlash.status() === 200) {
          ...
          console.warn(`⚠ T19: ${p} hem trailing-slash hem slash'sız 200 dönüyor (canonical ile çözülmeli)`);
        }
      }
  ...
  test('Duplicate title/meta description yok', async ({ page }) => {
    ...
    for (const pagePath of sitemapPaths) {
      try {
        ...
        const desc = await page.locator('meta[name="description"]').first().getAttribute('content').catch(() => '');
    ...
    for (const p of blogPaths.slice(0, 8)) {
      try {
        ...
        const wordCount = await page.evaluate(() => {
          ...
          return ((article as HTMLElement).innerText ?? article.textContent ?? '').trim().split(/\s+/).filter((w: string) => w.length > 2).length;
        });
        if (wordCount < 200) {
  ...
  test('T20: Blog yazıları topic cluster linkleri içeriyor', async ({ page }) => {
    ...
    const clusterLinks = await page.evaluate(() => {
      ...
      return links.filter(a => {
        ...
        return h.startsWith('/blog/') || h.startsWith('/services') || h.startsWith('/case-studies');
      }).length;
    });
  ...
  test('Services: her servis kartı kendi sayfasına link veriyor', async ({ page }) => {
    ...
    expect(serviceLinks.length, 'Services sayfasından /services/* linkleri eksik').toBeGreaterThan(0);
  });
  ...
  test('Pricing: CTA buton /contact veya booking\'a link veriyor', async ({ page }) => {
    ...
    const ctaLinks = await page.evaluate(() => {
      ...
      return links.filter(a => {
        ...
        return h.includes('contact') || h.includes('booking') || h.includes('book') ||
               text.includes('başla') || text.includes('görüşme') || text.includes('iletişim') ||
               text.includes('start') || text.includes('contact') || text.includes('get started');
      }).length;
    });
