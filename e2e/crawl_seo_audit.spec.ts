...
async function auditPage(page: Page, pagePath: string): Promise<SeoResult> {
  ...
  const hasJsonLd  = await page.locator('script[type="application/ld+json"]').count().then(c => c > 0);
  const hasHreflang = await page.locator('link[rel="alternate"][hreflang]').count().then(c => c > 0);
  const robotsContent = await page.locator('meta[name="robots"]').first().getAttribute('content').catch(() => null);
  ...
  // Title kontrolü
  if (!title) { issues.push('NO_TITLE'); score -= 25; }
  else if (titleLen < 30) { issues.push(`TITLE_SHORT (${titleLen})`); score -= 10; }
  else if (titleLen > 70) { issues.push(`TITLE_LONG (${titleLen})`); score -= 5; }

  // Meta description
  if (!metaDesc) { issues.push('NO_META_DESC'); score -= 20; }
  else if (metaDescLen < 70) { issues.push(`META_SHORT (${metaDescLen})`); score -= 10; }
  else if (metaDescLen > 165) { issues.push(`META_LONG (${metaDescLen})`); score -= 5; }

  // Canonical
  if (!canonical) { issues.push('NO_CANONICAL'); score -= 15; }

  // H1 sayısı
  if (h1Count === 0) { issues.push('NO_H1'); score -= 15; }
  else if (h1Count > 1) { issues.push(`MULTIPLE_H1 (${h1Count})`); score -= 10; }

  // JSON-LD (blog/case-study/pricing için zorunlu)
  const requiresJsonLd = ['/blog/', '/case-studies/', '/pricing', '/about', '/methodology'].some(p => pagePath.startsWith(p));
  if (requiresJsonLd && !hasJsonLd) { issues.push('NO_JSONLD'); score -= 10; }

  // robots
  if (robotsContent && robotsContent.includes('noindex')) { issues.push('NOINDEX_SET'); score -= 30; }

  // OG tags
  if (!ogTitle) { issues.push('NO_OG_TITLE'); score -= 5; }
  if (!ogDesc) { issues.push('NO_OG_DESC'); score -= 5; }

  return {
    path: pagePath, title, titleLen, metaDesc, metaDescLen, canonical,
    h1Count, hasJsonLd, hasHreflang, robotsContent, ogTitle, ogDesc,
...
test.describe('Crowler: SEO Full Audit (Sitemap Pages)', () => {
  ...
  for (const pagePath of sitemapPaths) {
    test(`SEO audit: ${pagePath}`, async ({ page }) => {
      ...
      if (!isAuthPage) {
        expect(result.h1Count, `[${pagePath}] H1 sayısı: ${result.h1Count} (beklenen: 1)`).toBeGreaterThanOrEqual(1);
      ...
      if (!isAuthPage) {
        ...
        expect(result.metaDescLen, `[${pagePath}] meta desc çok kısa (${result.metaDescLen})`).toBeGreaterThan(50);
      }
  ...
  test('SEO aggregate: ortalama skor ≥ 65/100', async () => {
    ...
    const avg = nonAuthResults.reduce((s, r) => s + r.score, 0) / Math.max(nonAuthResults.length, 1);
  ...
  test('Sitemap: tüm sayfalar erişilebilir (200 veya 3xx)', async ({ request }) => {
    ...
    for (const p of sitemapPaths.slice(0, 25)) { // İlk 25
  ...
  test('Canonical: öz-referans (self-referencing canonical)', async ({ page }) => {
    const criticalPaths = ['/', '/about', '/services', '/pricing', '/blog', '/contact'];
