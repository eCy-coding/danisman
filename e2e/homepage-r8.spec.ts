...
async function stubBackends(page: Page): Promise<void> {
  ...
  await page.route(/ingest\.sentry\.io|posthog\.com|google-analytics\.com|googletagmanager/, (route) =>
    route.fulfill({ status: 200, json: {} }),
  );
}
...
test.describe('Homepage R8 hardening', () => {
  ...
  test('R8-C5 — SuccessStories ArrowRight advances active card', async ({ page }) => {
    ...
    const carousel = page.getByRole('region', { name: /Başarı hikayeleri karuseli|Success stories carousel/ });
    ...
    const firstCardHref = await carousel.locator('a[href^="/case-studies/"]').first().getAttribute('href');
