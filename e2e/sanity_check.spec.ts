...
test('Production Audit: Capture Console Errors', async ({ page }) => {
  ...
  const filteredErrors = consoleErrors.filter(msg => 
    !BENIGN_PATTERNS.some(pattern => msg.includes(pattern))
  );
...
test('Booking Wizard E2E: submit to mock server', async ({ page }) => {
  ...
  await page.goto('http://localhost:4173/services', { waitUntil: 'domcontentloaded', timeout: 50000 });
  await page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => null);

  // Scroll to booking wizard to ensure it's in view
  await page.locator('#booking-wizard').scrollIntoViewIfNeeded().catch(() => null);
