...
router.get('/service/:slug', ...adminOnly, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.patch('/service/:slug', ...adminOnly, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const slug = req.params.slug;
    if (!slug) return res.status(400).json({ status: 'error', message: 'slug required' });
...
router.get('/page/:pageId', ...adminOnly, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.patch('/page/:pageId', ...adminOnly, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const pageId = req.params.pageId;
    if (!pageId) return res.status(400).json({ status: 'error', message: 'pageId required' });
    const payload = req.body as Record<string, unknown>;
