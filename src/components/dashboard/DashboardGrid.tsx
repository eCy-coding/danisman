...
const PlaceholderWidget = ({ title, color }: { title: string; color: string }) => (
  <div
    className={`h-full p-6 rounded-2xl bg-white/5 border border-white/10 shadow-sm flex flex-col ${color}`}
  >
...
export const DashboardGrid: React.FC = () => {
  ...
  const renderWidgetContent = (widget: Widget) => {
    switch (widget.type) {
      ...
      case 'activity':
        return <LiveTeamActivity />;
      case 'revenue':
        return <PlaceholderWidget title={widget.title} color="" />;
      case 'traffic':
        return <PlaceholderWidget title={widget.title} color="" />;
      case 'users':
        return <PlaceholderWidget title={widget.title} color="" />;
