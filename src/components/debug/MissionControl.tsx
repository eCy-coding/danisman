    ...
    <AnimatePresence>
      ...
      >
        <div className="flex items-center justify-between p-3 bg-slate-800 border-b border-slate-700">
          ...
          <button type="button" onClick={() => setIsVisible(false)} className="hover:text-white">
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          <div>
            ...
            <div className="space-y-1">
              {metrics.slice(-5).map((m: Metric, i: number) => (
                ...
                >
                  <span
                    className={`font-bold ${m.rating === 'poor' ? 'text-red-400' : m.rating === 'needs-improvement' ? 'text-yellow-400' : 'text-green-400'}`}
                  >
                    {m.name}
                  </span>
              ...
              {metrics.length === 0 && (
                <span className="text-slate-600 italic">Waiting for data...</span>
              )}
            </div>
          </div>
              ...
              {events
                ...
                .map((e: AnalyticsEvent, i: number) => (
                  <div key={i} className="flex gap-2 text-[10px] border-b border-slate-800 pb-1">
                    ...
                    <span
                      className={`uppercase font-bold ${e.type === 'error' ? 'text-red-400' : 'text-blue-300'}`}
                    >
                      [{e.type}]
                    </span>
