...
import { Check, ArrowRight, Activity, ShieldCheck, Zap, LineChart, FileText, Target } from 'lucide-react';
...
export const BusinessHealthQuiz: React.FC = () => {
    ...
    const getHealthTier = (score: number) => {
        if (score < 50) return { title: "Foundational Phase", color: "text-orange-400", bg: "bg-orange-500/10", stroke: "#fb923c" };
        if (score < 80) return { title: "Growth Phase", color: "text-blue-400", bg: "bg-blue-500/10", stroke: "#60a5fa" };
        return { title: "Optimization Phase", color: "text-emerald-400", bg: "bg-emerald-500/10", stroke: "#34d399" };
    };
                ...
                <div className="p-8 md:p-12 relative z-10 grid md:grid-cols-2 gap-12 items-center">
                    <div>
                        <div className="mb-8">
                            ...
                            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg ${tier.bg} ${tier.color} font-medium`}>
                        ...
                        <div className="bg-neutral-900/50 rounded-2xl p-6 border border-white/5 mb-8">
                            ...
                            <p className="text-slate-400 leading-relaxed text-sm">
                                Based on your multi-dimensional analysis, your organization shows strong potential but requires strategic alignment in 
                                {Object.entries(dimensionScores).sort((a,b) => a[1]-b[1])[0]?.[0] || 'Strategy'} to unlock the next stage of growth.
                            </p>
                        </div>
                    ...
                    {QUESTIONS.map((_, idx) => (
                        <div key={idx} className="flex-1 bg-white/10 rounded-full overflow-hidden">
                            <motion.div 
                                ...
                                initial={{ width: 0 }}
                                animate={{ width: idx < currentQuestionIdx ? '100%' : idx === currentQuestionIdx ? '50%' : '0%' }}
