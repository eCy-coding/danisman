  ...
  return (
    <Card className="p-6 space-y-4 bg-white/5 backdrop-blur-sm border-white/10">
      ...
      <div className="space-y-2">
        <label htmlFor={revenueId} className="text-sm font-medium text-gray-300">Yıllık Cironuz (₺)</label>
        <input
          id={revenueId}
          type="number"
          inputMode="numeric"
        ...
        />
        {errors.revenue && <span id={`${revenueId}-error`} className="text-red-400 text-xs">Bu alan zorunludur.</span>}
      </div>

      <div className="space-y-2">
        <label htmlFor={efficiencyId} className="text-sm font-medium text-gray-300">Hedeflenen Verimlilik Artışı (%)</label>
        <div className="relative">
          ...
          />
          <span className="absolute right-4 top-3 text-gray-400 font-bold" aria-hidden="true">%</span>
        </div>
        {errors.efficiencyGain && <span id={`${efficiencyId}-error`} className="text-red-400 text-xs">1 ile 100 arasında bir değer girin.</span>}
      </div>

      <div className="space-y-2">
        <label htmlFor={costId} className="text-sm font-medium text-gray-300">Tahmini Danışmanlık Bütçesi (₺)</label>
        <input
          id={costId}
          type="number"
          inputMode="numeric"
        ...
        />
        {errors.cost && <span id={`${costId}-error`} className="text-red-400 text-xs">Bu alan zorunludur.</span>}
      </div>

      <p className="text-xs text-slate-400 mt-2">
        * Tahminler sadece bilgi amaçlıdır.
      </p>
    </Card>
  );
};
