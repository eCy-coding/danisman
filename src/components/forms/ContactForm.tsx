  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        ...
        {errors.name && (
          <p className="text-red-500 text-xs flex items-center gap-1 mt-1">
            <AlertCircle size={12} /> {t(errors.name.message || 'contact.form.required')}
          </p>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        {errors.email && (
           <p className="text-red-500 text-xs flex items-center gap-1 mt-1">
             <AlertCircle size={12} /> {t(errors.email.message || 'contact.form.invalid_email')}
           </p>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <option value="partnership">{t('contact.form.subjects.partnership') || 'İş Ortaklığı'}</option>
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          className={`w-full px-4 py-3 rounded-lg border bg-white/5 text-white placeholder-slate-500 focus:ring-2 focus:ring-primary/20 outline-none transition-all resize-none ${
            errors.message ? 'border-red-400/50 bg-red-500/5' : 'border-white/10 focus:border-primary'
          }`}
          placeholder={t('contact.form.message_placeholder') || 'Size nasıl yardımcı olabiliriz?'}
        />
         {errors.message && (
           <p className="text-red-500 text-xs flex items-center gap-1 mt-1">
             <AlertCircle size={12} /> {t(errors.message.message || 'contact.form.required')}
           </p>
        )}
      </div>
      ...
      >
        {isSubmitting ? (
          <>
            <Loader2 size={20} className="animate-spin" /> {t('contact.form.sending') || 'Gönderiliyor...'}
          </>
        ) : (
          <>
      ...
      {submitStatus === 'success' && (
        <div className="p-4 bg-green-500/10 text-green-400 rounded-xl flex items-center gap-3 animate-fade-in">
          <CheckCircle size={24} />
          <div>
            <p className="font-bold">{t('contact.form.success_title') || 'Mesajınız İletildi!'}</p>
            <p className="text-sm">{t('contact.form.success_desc') || 'En kısa sürede size dönüş yapacağız.'}</p>
          </div>
        </div>
      )}

      {submitStatus === 'error' && (
        <div className="p-4 bg-red-500/10 text-red-400 rounded-xl flex items-center gap-3 animate-fade-in">
          <AlertCircle size={24} />
          <div>
            <p className="font-bold">{t('contact.form.error_title') || 'Bir Hata Oluştu'}</p>
            <p className="text-sm">{t('contact.form.error_desc') || 'Lütfen tekrar deneyiniz veya telefon ile ulaşınız.'}</p>
          </div>
        </div>
      )}
    </form>
