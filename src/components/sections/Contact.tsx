...
const ContactInfoItem: React.FC<{ icon: React.ReactNode; title: string; children: React.ReactNode; delay: number }> = ({ icon, title, children, delay }) => (
  <motion.div 
  ...
  >
    ...
    <div>
      <h3 className="font-medium text-white mb-1 group-hover:text-secondary transition-colors duration-300">{title}</h3>
    ...
    <div className="relative group">
      <label
        ...
        className={`absolute left-4 transition-all duration-300 pointer-events-none z-10 ${
          isFocused || value ? '-top-2.5 text-xs bg-surface px-2 text-secondary' : 'top-4 text-slate-400'
        }`}
      >
        {label} {required && '*'}
      </label>
      ...
      <div className={`absolute -inset-0.5 bg-linear-to-r from-secondary/50 to-primary/50 rounded-xl blur opacity-0 transition-opacity duration-500 ${isFocused ? 'opacity-30' : 'group-hover:opacity-10'}`} />
          ...
          <div>
            ...
            >
              ...
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-sans font-light text-white mb-8 leading-tight tracking-tight">
                Geleceği Birlikte <br /> <span className="text-secondary font-medium">Tasarlayalım</span>
              </h2>
              <p className="text-slate-400 text-lg lg:text-xl font-light mb-16 max-w-lg leading-relaxed">
                Projeleriniz için stratejik bir ortak arıyorsanız, uzman ekibimizle tanışın.
            ...
            <div className="space-y-8">
               <ContactInfoItem icon={<MapPin size={20} />} title="Merkez Ofis" delay={0.2}>
                  <p>Levent 199, Büyükdere Cd. No:199<br />34394 Şişli/İstanbul</p>
               </ContactInfoItem>
                ...
                {status === 'success' ? (
                  ...
                  >
                    <div className="w-20 h-20 rounded-full bg-green-500/10 border border-green-500/20 text-green-500 flex items-center justify-center mb-8 relative">
                      <div className="absolute inset-0 rounded-full border-t border-green-500 animate-spin opacity-50" style={{ animationDuration: '3s' }} />
                      <CheckCircle size={36} />
                    </div>
                      ...
                      <MagneticButton>
                        ...
                        >
                          ...
                          {!status.includes('submitting') && (
                            <Send size={18} className="relative z-10 group-hover:text-white group-hover:translate-x-1 group-hover:-translate-y-1 transition-all duration-500" />
                          )}
                        </button>
                      </MagneticButton>
                    </div>
