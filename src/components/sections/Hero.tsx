...
import { motion, useReducedMotion, useScroll, useTransform, useSpring, useMotionValue, Variants } from 'motion/react';
import { ArrowRight, Play, TrendingUp, Users, Shield, BarChart3, Code2, Cpu, Zap, LayoutDashboard } from 'lucide-react';
...
const PERSONA_CONTENT = {
  executive: {
    ...
    description: { tr: "Veriye dayalı içgörüler ve stratejik danışmanlık ile operasyonel maliyetleri %40 düşürün ve büyümeyi hızlandırın.", en: "Accelerate growth and reduce operational costs by 40% with data-driven insights and strategic consulting." },
    stats: [
      { icon: <TrendingUp size={20} />, value: "+340%", label: { tr: "Büyüme Hızı", en: "Growth Velocity" } },
      { icon: <Users size={20} />, value: "50M+", label: { tr: "Aktif Kullanıcı", en: "Active Users" } },
      { icon: <BarChart3 size={20} />, value: "99.99%", label: { tr: "Sistem Uptime", en: "System Uptime" } }
    ]
  },
  developer: {
    ...
    description: { tr: "Modern E2E test altyapısı, CI/CD pipeline optimizasyonu ve 'Tactile Brutalism' standartlarında yüksek performans.", en: "High performance with modern E2E test infrastructure, CI/CD pipeline optimization, and 'Tactile Brutalism' standards." },
    stats: [
      ...
      { icon: <Cpu size={20} />, value: "100%", label: { tr: "Test Kapsamı", en: "Test Coverage" } }
    ]
  }
};
...
const DataFlowBackground = () => {
  return (
    <div className="absolute inset-0 pointer-events-none opacity-40" aria-hidden="true">
      <svg width="100%" height="100%" preserveAspectRatio="none" className="absolute inset-0" aria-hidden="true">
        <defs>
        ...
        {/* Network Lines */}
        <path d="M 0 200 Q 300 100 600 300 T 1200 200 T 1800 400" fill="none" stroke="url(#flow-gradient)" strokeWidth="1" strokeDasharray="5 5" />
        <path d="M 0 600 Q 400 800 800 500 T 1500 700 T 2000 500" fill="none" stroke="url(#flow-gradient-2)" strokeWidth="1" strokeDasharray="5 5" />
        <path d="M 300 0 L 600 300 L 800 500 L 1200 200" fill="none" stroke="url(#flow-gradient)" strokeWidth="0.5" opacity="0.5" />
        
        {/* Animated Particles along paths */}
        <motion.circle r="3" fill="#38BDF8" filter="url(#glow)"
          initial={{ cx: 0, cy: 200 }}
          animate={{
        ...
        />
        <motion.circle r="3" fill="#2563EB" filter="url(#glow)"
          initial={{ cx: 0, cy: 600 }}
          animate={{
...
export const Hero: React.FC = () => {
  ...
  return (
    <section id="hero" ref={containerRef} className="relative min-h-[140vh] bg-[#050810] pt-28 pb-32">
      ...
      <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 w-full relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center mt-8">
          ...
          <MotionOrDiv
            ...
            {...(!prefersReducedMotion ? { variants: containerVariants, initial: 'hidden', animate: 'visible' } : {})}
            className="col-span-1 lg:col-span-7 flex flex-col justify-center"
          >
            <motion.div variants={itemVariants} className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full border border-primary/30 bg-primary/5 backdrop-blur-md mb-8 w-fit">
            ...
            <motion.p variants={itemVariants} className="text-lg md:text-xl text-slate-300/90 leading-relaxed max-w-2xl mb-12 font-light border-l-4 border-secondary/50 pl-6 backdrop-blur-sm bg-white/2 py-2 rounded-r-lg min-h-20">
            ...
            <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-5 items-center">
              <MagneticButton strength={40}>
                <a href="#contact" data-testid="hero-cta-primary" onClick={(e) => handleCtaClick(e, '#contact', 'Hero Primary')} className="group relative px-8 py-4 bg-primary hover:bg-blue-600 text-white font-bold uppercase tracking-widest rounded-xl shadow-[0_0_40px_rgba(37,99,235,0.3)] hover:shadow-[0_0_60px_rgba(37,99,235,0.5)] transition-all duration-300 flex items-center justify-center gap-3 min-w-50 w-full sm:w-auto overflow-hidden border border-white/10">
                  ...
                  <span className="relative z-10">{lang === 'tr' ? 'Hemen Başlayın' : 'Get Started'}</span>
                  <ArrowRight size={18} className="relative z-10 group-hover:translate-x-1 transition-transform" />
                </a>
              </MagneticButton>

              <MagneticButton strength={20}>
                <a href="#services" data-testid="hero-cta-secondary" onClick={(e) => handleCtaClick(e, '#services', 'Hero Secondary')} className="group px-8 py-4 bg-white/5 hover:bg-white/10 border border-white/10 text-slate-200 text-sm font-bold uppercase tracking-widest rounded-xl backdrop-blur-md transition-all duration-300 flex items-center justify-center gap-3 min-w-50 w-full sm:w-auto">
                  <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                    <Play size={14} fill="currentColor" className="opacity-90 ml-0.5" />
                  </div>
                  {lang === 'tr' ? 'Platformu İncele' : 'Explore Platform'}
                </a>
              </MagneticButton>
            </motion.div>
          </MotionOrDiv>
      ...
      >
        {[
          { icon: <LayoutDashboard size={24} />, title: {tr: "Unified Dashboard", en: "Unified Dashboard"}, desc: {tr: "Tüm metrikleri tek ekranda izleyin. Karar alma süreçlerinizi anında hızlandırın.", en: "Monitor all metrics in one screen. Accelerate your decision-making processes instantly."} },
          { icon: <Shield size={24} />, title: {tr: "Kurumsal Güvenlik", en: "Enterprise Security"}, desc: {tr: "Sıfır güvenlik açığı, tam uyumluluk. Verileriniz askeri düzeyde korunur.", en: "Zero vulnerabilities, full compliance. Your data is protected at military grade."} },
          { icon: <Zap size={24} />, title: {tr: "Real-time Analitik", en: "Real-time Analytics"}, desc: {tr: "Milisaniyeler içinde kararlar alın. Pazar değişimlerine anında tepki verin.", en: "Make decisions in milliseconds. React to market changes instantly."} }
        ].map((feature, i) => (
          <div key={i} className="bg-[#0A0F1C]/80 border border-white/5 p-8 rounded-2xl backdrop-blur-xl shadow-2xl hover:border-primary/30 transition-colors duration-500 group">
            ...
            <h2 className="text-xl font-bold text-white mb-3 tracking-tight">{feature.title[lang]}</h2>
    ...
    <FloatingElement delay={delay} amplitude={8} className={className}>
      ...
      >
        ...
        <div className="flex items-center justify-between" style={{ transform: "translateZ(20px)" }}>
          ...
          <motion.div 
            initial={{ width: 0 }} animate={{ width: "45%" }} transition={{ duration: 1.5, delay: delay + 0.5, ease: "easeInOut" }}
        ...
        <div style={{ transform: "translateZ(30px)" }}>
          ...
          <span className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-2 block">{label}</span>
        </div>
      </motion.div>
    </FloatingElement>
  );
