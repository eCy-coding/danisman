...
const SUCCESS_STORIES_COPY = {
  ...
  description: { tr: 'Müşterilerimizle birlikte elde ettiğimiz ölçülebilir başarılar ve büyüme hikayeleri.', en: 'Measurable successes and growth stories we achieved together with our clients.' },
                ...
                <article className="relative bg-white/5 rounded-3xl overflow-hidden h-full border border-white/10 shadow-2xl hover:border-secondary/30 transition-all duration-500 flex flex-col group">
                  ...
                  <div className="p-8 flex flex-col flex-grow bg-transparent relative z-10">
                    ...
                    <div className="mt-auto pt-4 border-t border-white/5 flex items-center">
                      ...
                      <span className="ml-2 text-secondary group-hover:translate-x-2 transition-transform duration-300">→</span>
                    </div>
                  </div>
                </article>
              </Link>
                ...
                <Link to={`/case-studies/${study.slug}`} className="block h-full outline-none group">
                  <article className="relative bg-white/5 rounded-3xl overflow-hidden h-full border border-white/10 flex flex-col">
                    ...
                    <div className="p-6 flex flex-col flex-grow">
                      <div className="text-3xl font-sans font-light text-white mb-2 group-hover:text-secondary transition-colors">{study.result[lang]}</div>
                      <h3 className="text-lg font-medium text-slate-200 mb-2">{study.client}</h3>
                      <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">{study.category[lang]}</div>
                      <p className="text-slate-400 text-sm leading-relaxed mb-6 font-light line-clamp-3">{study.description[lang]}</p>
                      <div className="mt-auto pt-4 border-t border-white/5 flex items-center">
                        <span className="text-xs font-medium text-white uppercase tracking-wider group-hover:text-secondary">{SUCCESS_STORIES_COPY.details[lang]}</span>
