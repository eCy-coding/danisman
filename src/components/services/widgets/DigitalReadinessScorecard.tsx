...
export const DigitalReadinessScorecard: React.FC = () => {
  ...
  const rec = score < 4 ? 'Dijital Olgunluk Audit (2 hafta)' : score < 7 ? 'ERP/RPA + Tech Strategy' : 'Operating Model + Scaling';

  return (
    <section className="py-16 px-6 md:px-12 border-t border-white/5 bg-gradient-to-b from-transparent via-secondary/[0.03] to-transparent" aria-labelledby="dig-readiness-heading" data-testid="digital-readiness-scorecard">
      <div className="max-w-4xl mx-auto">
        <div className="mb-10">
          <div className="text-xs font-bold uppercase tracking-[0.25em] text-secondary mb-3">Dijital Hazırlık Skoru</div>
          <h2 id="dig-readiness-heading" className="text-3xl md:text-4xl font-serif font-bold text-white mb-3">10 Maddelik Hızlı Audit</h2>
          <p className="text-slate-400">Her madde için Evet/Hayır seçin — toplam skor + gap önerisi.</p>
        </div>
          ...
          {ITEMS.map((item, i) => (
            <div key={i} className="flex items-center gap-4 p-4 bg-white/5 border border-white/10 rounded-xl">
              ...
              <div className="flex gap-2 flex-shrink-0">
                <button type="button" onClick={() => { const next = [...answers]; next[i] = true; setAnswers(next); }}
                  ...
                  aria-pressed={answers[i] === true} aria-label={`${item}: Evet`}><Check size={14} /></button>
                <button type="button" onClick={() => { const next = [...answers]; next[i] = false; setAnswers(next); }}
                  ...
                  aria-pressed={answers[i] === false} aria-label={`${item}: Hayır`}><X size={14} /></button>
              </div>
            </div>
          ))}
        </div>
        {completed && (
          <div className="bg-gradient-to-br from-secondary/15 to-primary/10 border border-secondary/30 rounded-2xl p-6 md:p-8 animate-in fade-in slide-in-from-bottom-4 duration-500" role="status" aria-live="polite">
            ...
            <div className="flex items-baseline gap-2 mb-4"><span className="text-5xl font-serif font-bold text-white">{score}</span><span className="text-slate-400">/ 10</span></div>
            <p className="text-slate-200 leading-relaxed mb-6">Önerilen engagement: <strong className="text-white">{rec}</strong></p>
