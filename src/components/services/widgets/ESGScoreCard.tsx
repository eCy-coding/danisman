...
const PILLARS = [
  {
    ...
    color: 'emerald',
    items: [
      'Scope 1+2 karbon envanteri var',
      'Scope 3 ölçümü başlatıldı',
      'Enerji verimlilik hedefi yazılı',
      'Atık/su yönetim politikası var',
      'Yeşil tedarik kriterleri uygulanıyor',
    ],
  },
  {
    id: 'S',
    ...
    color: 'amber',
    items: [
      'Çalışan engagement yıllık ölçülür',
      'Çeşitlilik (D&I) politikası aktif',
      'Tedarikçi insan hakları audit',
      'Topluluk yatırım programı',
      'İş güvenliği (LTIR) hedef altında',
    ],
  },
  {
    id: 'G',
    ...
    items: [
      ...
      'ESG komitesi kurulu',
      'Etik kod + ihlal hattı',
      'Risk yönetim framework yazılı',
      'Sürdürülebilirlik raporu yıllık',
    ],
  },
];
...
export const ESGScoreCard: React.FC = () => {
  ...
  const rec =
    pct < 30
      ? 'ESG Baseline Audit (3 hafta)'
      : pct < 60
        ? 'Reporting Framework + Materiality'
    ...
    >
      <div className="max-w-5xl mx-auto">
        <div className="mb-10">
          ...
          >
            E/S/G — 15 Maddelik Audit
          </h2>
          <p className="text-slate-400">
            Her boyutta evet/hayır işaretleyin. Toplam ESG readiness % + GAP raporu.
          </p>
        </div>
                ...
                <div className="space-y-2">
                  {p.items.map((item, i) => {
                    ...
                    return (
                      <label
                        key={i}
                        className="flex items-start gap-2 text-sm cursor-pointer min-h-[44px]"
                      >
                        <input
                          type="checkbox"
                          checked={checked}
        ...
        {allAnswered && (
          <div
            className="bg-gradient-to-br from-emerald-500/15 to-secondary/10 border border-emerald-500/30 rounded-2xl p-6 md:p-8 animate-in fade-in slide-in-from-bottom-4 duration-500"
            role="status"
            aria-live="polite"
          >
            ...
            <Link
              to="/contact"
              className="inline-flex items-center gap-2 px-6 py-3 min-h-[44px] rounded-xl bg-emerald-500 text-neutral font-semibold hover:bg-emerald-400 transition-colors"
            >
              ESG Audit Görüşmesi <ArrowRight size={16} />
            </Link>
          </div>
        )}
      </div>
