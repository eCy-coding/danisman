...
export const EmployerBrandHealth: React.FC = () => {
  ...
  const rec =
    score < 50
      ? 'EVP Audit + Brand Redesign'
      : score < 75
        ? 'Recruitment Marketing'
        : 'Amplifier — Glassdoor optimization';
  ...
  return (
    <section
      ...
      data-testid="employer-brand-health"
    >
      <div className="max-w-6xl mx-auto">
        <div className="mb-10">
          <div className="text-xs font-bold uppercase tracking-[0.25em] text-secondary mb-3">
            İşveren Markası Sağlığı
          </div>
          <h2 id="eb-heading" className="text-3xl md:text-4xl font-serif font-bold text-white mb-3">
            8 Boyutlu Brand Health
          </h2>
          <p className="text-slate-400">
            Her boyutu mevcut durumunuza göre ayarlayın. Toplam sağlık skoru + zayıf alanlar.
          </p>
        </div>
            ...
            return (
              <div key={d.id} className="bg-white/5 border border-white/10 rounded-2xl p-5">
                <div className="flex justify-between items-baseline mb-2">
                  ...
                  <span
                    className={`text-xs font-bold px-2 py-0.5 rounded ${norm >= 75 ? 'bg-emerald-500/20 text-emerald-400' : norm >= 50 ? 'bg-amber-500/20 text-amber-400' : 'bg-red-500/20 text-red-400'}`}
                  >
                    {norm}/100
                  </span>
                </div>
        ...
        <div className="bg-gradient-to-br from-secondary/15 to-primary/10 border border-secondary/30 rounded-2xl p-6 md:p-8">
          ...
          >
            EVP Audit Görüşmesi <ArrowRight size={16} />
          </Link>
        </div>
      </div>
    </section>
