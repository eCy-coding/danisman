...
export const EmploymentIncentiveCalculator: React.FC = () => {
  ...
  const programs = useMemo(() => {
    ...
    if (sector === 'Teknoloji/Yazılım')
      list.push({ name: '5746 Ar-Ge SGK Teşviki', saving: workforce * avgSalary * 0.155 });
    // 4447 - Genç istihdam
    list.push({ name: '4447 Genç İstihdam (tahmini)', saving: workforce * 0.2 * avgSalary * 0.12 });
    // 7256 - Yeni istihdam (5+ kişi şartı)
    if (workforce >= 5)
      list.push({
        name: '7256 Yeni İstihdam Desteği',
        saving: workforce * 0.1 * avgSalary * 0.155,
      });
    // 6. Bölge SGK
    if (region === '6. Bölge')
      list.push({ name: '6. Bölge SGK Devlet Desteği', saving: workforce * avgSalary * 0.155 });
    // 4-5. Bölge teşvik
    if (region === '4-5. Bölge')
      list.push({ name: '4-5. Bölge Bölgesel Teşvik', saving: workforce * avgSalary * 0.08 });
    return list;
  ...
  return (
    <section
      ...
      aria-labelledby="payroll-heading"
      data-testid="employment-incentive-calculator"
    >
      <div className="max-w-5xl mx-auto">
        <div className="mb-10">
          <div className="text-xs font-bold uppercase tracking-[0.25em] text-secondary mb-3">
            İstihdam Teşvik Tahminleyici
          </div>
          <h2
            id="payroll-heading"
            className="text-3xl md:text-4xl font-serif font-bold text-white mb-3"
          >
            SGK Teşvik Tasarruf Potansiyeli
          </h2>
          <p className="text-slate-400">
            Bordro değerlerinizi girin — uygun olduğunuz teşvik programları + tahmini yıllık
            tasarruf.
          </p>
        </div>
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-gradient-to-br from-secondary/15 to-primary/10 border border-secondary/30 rounded-2xl p-6 md:p-8">
            <div className="text-xs uppercase tracking-widest text-secondary mb-3">
              Uygun Programlar ({programs.length})
            </div>
            {programs.length === 0 ? (
              <p className="text-slate-300 text-sm mb-4">
                Mevcut girdilerinizle uygulanabilir program bulunmadı.
              </p>
            ) : (
              <ul className="space-y-2 mb-6">
                {programs.map((p) => (
                  <li
                    key={p.name}
                    className="flex justify-between items-start gap-2 text-sm bg-white/5 p-3 rounded-lg"
                  >
                    <span className="text-slate-200 flex items-start gap-2">
                      <CheckCircle2 size={14} className="text-emerald-400 mt-0.5 flex-shrink-0" />
                      {p.name}
                    </span>
                    <span className="text-secondary font-serif font-bold whitespace-nowrap">
                      ₺{Math.round(p.saving / 1000)}k/yıl
                    </span>
                  </li>
                ))}
              </ul>
            ...
            <div className="pt-4 border-t border-white/10">
              ...
              <div className="text-3xl md:text-4xl font-serif font-bold text-white mb-4">
