...
export const GenerationalTransitionTimeline: React.FC = () => {
  ...
  return (
    <section
      ...
      aria-labelledby="gen-timeline-heading"
      data-testid="generational-transition-timeline"
    >
      <div className="max-w-6xl mx-auto">
        <div className="mb-10">
          <div className="text-xs font-bold uppercase tracking-[0.25em] text-secondary mb-3">
            Nesil Geçişi Teşhisi
          </div>
          <h2
            id="gen-timeline-heading"
            className="text-3xl md:text-4xl font-serif font-bold text-white mb-3"
          >
            Aile Şirketi Nesil Haritası
          </h2>
          <p className="text-slate-400 max-w-2xl">
            Şirketinizin bugün hangi nesil aşamasında olduğunu seçin — typical pain + governance gap
            + intervention paketi.
          </p>
        </div>
          ...
          {GENS.map((gen) => {
            ...
            return (
              ...
              >
                <Users
                  size={20}
                  className={`mb-3 ${isSelected ? 'text-secondary' : 'text-slate-400'}`}
                />
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              <div>
                <div className="text-xs font-bold uppercase tracking-widest text-amber-400 mb-3 flex items-center gap-2">
                  <AlertCircle size={14} /> Tipik Pain
                </div>
                <p className="text-slate-200 text-sm leading-relaxed mb-4">{selectedGen.pain}</p>
                <p className="text-slate-300 text-sm leading-relaxed italic">
                  Governance gap: {selectedGen.gap}
                </p>
              </div>
              <div>
                <div className="text-xs font-bold uppercase tracking-widest text-emerald-400 mb-3 flex items-center gap-2">
                  <CheckCircle2 size={14} /> Intervention
                </div>
                <p className="text-slate-200 text-sm leading-relaxed mb-6">
                  {selectedGen.intervention}
                </p>
                <Link
                  to="/contact"
