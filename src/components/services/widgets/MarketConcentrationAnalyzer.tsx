...
export const MarketConcentrationAnalyzer: React.FC = () => {
  ...
  const verdict = useMemo(() => {
    if (hhi >= 2500)
      return {
        tone: 'critical',
        label: 'Yüksek Konsantrasyon',
        text: 'HHI ≥ 2500 — antitröst inceleme riski yüksek. Birleşme/işbirliği işlemlerinde detaylı ekonomik analiz zorunlu.',
      };
    if (hhi >= 1500)
      return {
        tone: 'warning',
        label: 'Orta Konsantrasyon',
        text: 'HHI 1500-2500 — Rekabet Kurumu bildirim eşikleri test edilmeli; market definition tartışılabilir.',
      };
    return {
      tone: 'ok',
      label: 'Düşük Konsantrasyon',
      text: 'HHI < 1500 — pazar yeterince rekabetçi. Antitröst riski düşük.',
    };
  }, [hhi]);
  ...
  return (
    <section
      ...
      aria-labelledby="hhi-heading"
      data-testid="market-concentration-analyzer"
    >
      <div className="max-w-5xl mx-auto">
        <div className="mb-10">
          ...
          >
            Pazar Konsantrasyonu Analizi
          </h2>
          <p className="text-slate-400">
            Pazarın en büyük 5 oyuncusunun pazar payını girin. Herfindahl-Hirschman Index +
            regülasyon exposure değerlendirmesi.
          </p>
        </div>
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 md:p-8 space-y-4">
            {shares.map((sh, i) => (
              <div key={i}>
                ...
                >
                  Oyuncu {i + 1} Pazar Payı:{' '}
                  <span className="text-secondary font-serif">%{sh}</span>
                </label>
                <input
                  id={`share-${i}`}
                  type="range"
            ...
            <p className="text-xs text-slate-400 pt-2 border-t border-white/5">
              Toplam: %{total}{' '}
              {total > 100 && <span className="text-amber-400">({'>'}100, gerçekçi değil)</span>}
            </p>
          </div>
          <div
          ...
          >
            ...
            <p
              className={`font-semibold mb-4 ${verdict.tone === 'critical' ? 'text-red-400' : verdict.tone === 'warning' ? 'text-amber-400' : 'text-emerald-400'}`}
            >
              {verdict.tone === 'ok' ? (
            ...
            >
              Antitröst Audit <ArrowRight size={16} />
            </Link>
          </div>
        </div>
      </div>
