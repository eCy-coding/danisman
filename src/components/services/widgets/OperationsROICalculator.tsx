...
export const OperationsROICalculator: React.FC = () => {
  ...
  const annualSavings = useMemo(
    () => ((revenue * costPct) / 100) * (savingsPct / 100),
    [revenue, costPct, savingsPct],
  );
  ...
  return (
    <section
      ...
      aria-labelledby="ops-roi-heading"
      data-testid="operations-roi-calculator"
    >
      <div className="max-w-5xl mx-auto">
        <div className="mb-10">
          <div className="text-xs font-bold uppercase tracking-[0.25em] text-secondary mb-3 flex items-center gap-2">
            <Calculator size={14} /> İnteraktif Hesaplayıcı
          </div>
          <h2
            id="ops-roi-heading"
            className="text-3xl md:text-4xl font-serif font-bold text-white mb-3"
          >
            Lean ROI Tahminleyici
          </h2>
          <p className="text-slate-400 max-w-2xl">
            Şirketinizin değerlerini girin; tipik Lean Six Sigma engagement'ın 12 aylık tahmini
            tasarrufunu görün.
          </p>
        </div>
        <div className="grid md:grid-cols-2 gap-8">
          <div className="space-y-6 bg-white/5 border border-white/10 rounded-2xl p-6 md:p-8">
            ...
            <div>
              <label htmlFor="ops-cost" className="block text-sm font-medium text-slate-200 mb-2">
                Operasyonel Maliyet Oranı (%)
              </label>
              <input
                id="ops-cost"
                type="range"
            ...
            <div>
              <label
                htmlFor="ops-maturity"
                className="block text-sm font-medium text-slate-200 mb-2"
              >
                Mevcut Lean Olgunluk (1-5)
              </label>
              <input
                id="ops-maturity"
                type="range"
              ...
              <p className="mt-2 text-sm text-slate-400">
                {['Hiç yok', 'Başlangıç', 'Gelişiyor', 'Olgun', 'Optimize'][maturity - 1]}
              </p>
            </div>
          </div>
          <div className="bg-gradient-to-br from-secondary/15 to-primary/10 border border-secondary/30 rounded-2xl p-6 md:p-8 flex flex-col justify-between">
            <div>
              <div className="text-xs font-bold uppercase tracking-widest text-secondary mb-3">
                Tahmini 12 Aylık Sonuç
              </div>
              <div className="space-y-5">
                <div>
                  <div className="text-4xl md:text-5xl font-serif font-bold text-white mb-1">
                    {formatTRY(annualSavings)}
                  </div>
                  <p className="text-slate-300 text-sm">
                    Yıllık tasarruf potansiyeli (%{savingsPct.toFixed(1)} OPEX reduction)
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-slate-300 text-sm">
                    Engagement ödeme süresi (tipik ₺500k engagement)
                  </p>
                </div>
              </div>
              <p className="text-xs text-slate-400 mt-6 italic">
                * Bu hesaplama indikatif. Gerçek sonuç sektör, başlangıç olgunluk + uygulama
                disiplinine bağlıdır.
              </p>
            </div>
            <Link
              to="/contact"
              className="inline-flex items-center gap-2 mt-6 px-6 py-3 min-h-[44px] rounded-xl bg-secondary text-neutral font-semibold hover:bg-secondary/90 transition-colors"
            >
