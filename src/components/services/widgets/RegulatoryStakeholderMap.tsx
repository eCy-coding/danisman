...
const QUADRANTS = {
  'high-high': {
    name: 'Yakın Yönet',
    desc: 'Sürekli engagement + briefing + position paper. Aylık görüşme + krizleri öngör.',
  },
  'high-low': {
    name: 'Tatmin Et',
    desc: 'Periodic update + bilgi paylaşımı. Reaktif değil proactive bilgilendirme.',
  },
  'low-high': {
    name: 'Bilgilendir',
    desc: 'Newsletter + dernek aracılığıyla sürekli iletişim. Sahibimiz değil ama destekçi.',
  },
...
export const RegulatoryStakeholderMap: React.FC = () => {
  ...
  return (
    <section
      ...
      data-testid="regulatory-stakeholder-map"
    >
      <div className="max-w-6xl mx-auto">
        <div className="mb-10">
          ...
          <h2 id="gr-heading" className="text-3xl md:text-4xl font-serif font-bold text-white mb-3">
            Influence × Interest Grid
          </h2>
          <p className="text-slate-400 max-w-2xl">
            8 örnek stakeholder'ı kendi gözlemlerinize göre konumlandırın (1-5 etki + 1-5 ilgi). Her
            quadrant için engagement stratejisi.
          </p>
        </div>
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
            <div className="text-xs font-bold uppercase tracking-widest text-secondary mb-4">
              Stakeholder Ayarları
            </div>
                ...
                <div key={s.id} className="text-sm">
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-xs text-slate-400">
                      Etki {s.influence} · İlgi {s.interest}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      max="5"
                      value={s.influence}
                      onChange={(e) => update(s.id, 'influence', Number(e.target.value))}
                      className="accent-secondary"
                      aria-label={`${s.name} etki`}
                    />
                    <input
                      type="range"
                      min="1"
                      max="5"
                      value={s.interest}
                      onChange={(e) => update(s.id, 'interest', Number(e.target.value))}
                      className="accent-emerald-400"
                      aria-label={`${s.name} ilgi`}
                    />
                  </div>
                </div>
              ))}
              ...
              return (
                <div key={q} className="bg-white/5 border border-white/10 rounded-xl p-4">
                  ...
                  <ul className="space-y-1">
                    {list.length === 0 ? (
                      <li className="text-xs text-slate-500 italic">Yok</li>
                    ) : (
                      list.map((s) => (
        ...
        <div className="bg-gradient-to-br from-secondary/15 to-primary/10 border border-secondary/30 rounded-2xl p-6 md:p-8 text-center">
          <p className="text-slate-200 mb-4">
            Yakın Yönet ({grouped['high-high']?.length ?? 0}) stakeholder için aylık briefing +
            position paper portföy.
          </p>
          <Link
            to="/contact"
          ...
          >
            GR Strateji Görüşmesi <ArrowRight size={16} />
          </Link>
        </div>
      </div>
    </section>
