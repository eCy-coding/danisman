...
export const UnionEngagementMatrix: React.FC = () => {
  ...
  const strategy = useMemo(() => {
    ...
    const labels = {
      high: {
        name: 'Stratejik Müzakere + Pre-emptive',
        desc: 'Yıllık ortaklık modu — sürekli sendika diyaloğu, pre-emptive issue resolution, çalışma barışı KPI dashboard.',
      },
      medium: {
        name: 'Çeyreklik Müzakere Hazırlık',
        desc: 'TİS dönemleri için BATNA + mali kapasite analizi + senaryo tatbikatı.',
      },
      low: {
        name: 'Audit + Compliance',
        desc: 'Reaktif değil proactive — yıllık İSG audit, sözleşme review.',
      },
    };
    return labels[risk];
  }, [unionPct, strike]);
  ...
  return (
    <section
      ...
      aria-labelledby="union-heading"
      data-testid="union-engagement-matrix"
    >
      <div className="max-w-5xl mx-auto">
        <div className="mb-10">
          <div className="text-xs font-bold uppercase tracking-[0.25em] text-secondary mb-3">
            Endüstriyel İlişkiler Profili
          </div>
          <h2
            id="union-heading"
            className="text-3xl md:text-4xl font-serif font-bold text-white mb-3"
          >
            Sendika Müzakere Stratejisi
          </h2>
          <p className="text-slate-400">
            Sektör + çalışan sayısı + sendikalaşma oranı + grev geçmişi → uygun müzakere stratejisi.
          </p>
        </div>
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 md:p-8 space-y-5">
            <div>
              <label htmlFor="ir-sector" className="block text-sm font-semibold text-white mb-2">
                Sektör
              </label>
              <select
                id="ir-sector"
            ...
            <div>
              <label htmlFor="ir-wf" className="block text-sm font-semibold text-white mb-2">
            ...
            <div>
              <label htmlFor="ir-union" className="block text-sm font-semibold text-white mb-2">
                Sendikalaşma Oranı: <span className="text-secondary font-serif">%{unionPct}</span>
              </label>
              <input
                id="ir-union"
                type="range"
                ...
                step="5"
                value={unionPct}
                onChange={(e) => setUnionPct(Number(e.target.value))}
                className="w-full accent-secondary"
              />
            </div>
            <div>
              <label htmlFor="ir-strike" className="block text-sm font-semibold text-white mb-2">
                Grev Geçmişi
              </label>
              <select
                id="ir-strike"
          ...
          <div className="bg-gradient-to-br from-secondary/15 to-primary/10 border border-secondary/30 rounded-2xl p-6 md:p-8">
            ...
            >
              TİS Hazırlık Görüşmesi <ArrowRight size={16} />
            </Link>
          </div>
        </div>
      </div>
