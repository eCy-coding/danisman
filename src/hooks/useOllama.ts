...
import { OllamaClient, type OllamaChatMessage, type OllamaOptions, OllamaError } from '../lib/ollama-client';
...
export function useOllama(opts: UseOllamaOptions = {}): UseOllamaReturn {
  const {
    model = (import.meta.env.VITE_OLLAMA_DEFAULT_MODEL as string | undefined) ?? 'qwen2.5-coder:14b',
    onChunk,
    onComplete,
    onError,
    ...clientOpts
  ...
  const handleError = useCallback((err: unknown) => {
    const msg = err instanceof OllamaError
      ? err.message
  ...
  const generate = useCallback(async (prompt: string, system?: string): Promise<string> => {
    setIsLoading(true);
    setError(null);
  ...
  const chat = useCallback(async (messages: OllamaChatMessage[]): Promise<string> => {
    setIsLoading(true);
    setError(null);
    abortRef.current = new AbortController();
    try {
  ...
  const generateStream = useCallback(async (prompt: string, system?: string): Promise<void> => {
    setIsStreaming(true);
    setStreamText('');
    setError(null);
    abortRef.current = new AbortController();
  ...
  const chatStream = useCallback(async (messages: OllamaChatMessage[]): Promise<void> => {
    setIsStreaming(true);
    setStreamText('');
    setError(null);
    abortRef.current = new AbortController();
