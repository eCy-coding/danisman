    ...
    <div className="min-h-screen bg-neutral text-slate-300">
      ...
      <section className="py-12 px-6 md:px-12 grid md:grid-cols-3 gap-10 max-w-6xl mx-auto">
        ...
        <div className="md:col-span-2 space-y-8">
          <div>
            <h2 className="text-xs font-bold uppercase tracking-widest text-secondary mb-3">Bio (50 kelime)</h2>
            <p className="text-slate-200 leading-relaxed">{BIO_50}</p>
          </div>
          <div>
            <h2 className="text-xs font-bold uppercase tracking-widest text-secondary mb-3">Bio (150 kelime)</h2>
            <p className="text-slate-200 leading-relaxed">{BIO_150}</p>
          </div>
          <div>
            <h2 className="text-xs font-bold uppercase tracking-widest text-secondary mb-3">Bio (300 kelime)</h2>
      ...
      <section className="py-12 px-6 md:px-12 border-t border-white/5 bg-white/[0.02]">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-serif font-bold text-white mb-8">İndirilebilir Varlıklar</h2>
          <div className="grid md:grid-cols-3 gap-5">
            {[
              { name: 'Logo Paketi (SVG + PNG)', file: '/brand/logo-horizontal.svg', desc: 'Yatay, dikey, mono ve favicon varyantları' },
              { name: 'Brand Guidelines', file: '/brand/BRAND_GUIDE.md', desc: 'Renk palette, typography, anatomi' },
              { name: 'Founder Portrait (HD)', file: '/founder.jpg', desc: '1200×1200, 300dpi (yoksa SVG fallback)' },
              { name: 'OG Share Image', file: '/brand/og-share.svg', desc: '1200×630 sosyal share' },
              { name: 'Icon Mark', file: '/brand/icon-mark.svg', desc: 'Standalone square icon' },
              { name: 'Stacked Logo', file: '/brand/logo-stacked.svg', desc: '200×220 vertical lock-up' },
            ].map((asset) => (
              <a
                key={asset.file}
                href={asset.file}
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className="text-[10px] uppercase tracking-widest text-slate-500 font-bold">İndir</span>
                </div>
      ...
      <section className="py-20 px-6 md:px-12 border-t border-white/5">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-2xl md:text-3xl font-serif font-bold text-white mb-5">Medya İletişimi</h2>
          <p className="text-slate-400 mb-8 leading-relaxed">
            Röportaj, panel davet, podcast konuğu, expert quote talepleri için. Aynı gün içinde dönüyoruz.
          </p>
