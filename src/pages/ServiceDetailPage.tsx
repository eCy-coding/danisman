...
export const ServiceDetailPage: React.FC = () => {
  ...
  const relatedPosts = React.useMemo(() => {
    ...
    const parentCategory = SERVICE_CATEGORIES.find(cat => 
        cat.items.some((item: { title: { en: string } }) => item.title['en'] === serviceItem?.title?.['en'])
    );
    const categoryId = parentCategory?.id; 

    if (!categoryId) return [];

    return blogPosts.filter((post: { serviceCategories?: string[] }) =>  
  ...
  const serviceUrl = `https://ecypro.com/services/${slug}`;
  ...
  return (
    <div className="min-h-screen bg-neutral flex flex-col font-sans text-slate-300 selection:bg-secondary selection:text-white">
      ...
      <JsonLd data={buildBreadcrumbSchema([
        { name: language === 'tr' ? 'Anasayfa' : 'Home', url: 'https://ecypro.com/' },
        { name: language === 'tr' ? 'Hizmetler' : 'Services', url: 'https://ecypro.com/services' },
      ...
      <div className="grow pb-16 px-6 md:px-12 max-w-7xl mx-auto w-full">
        <div className="mb-8">
            <Link to="/#services" className="inline-flex items-center text-slate-400 hover:text-primary transition-colors mb-4 text-sm font-medium">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2 space-y-16">
                <div className="space-y-8">
                    <div>
                         ...
                         <ServiceLiveTracker serviceId={getLang(serviceItem.title, 'en').toLowerCase().replace(/\s+/g, '-')} />
                    ...
                    <div className="prose prose-invert max-w-none text-slate-400">
                        <h3 className="text-2xl font-bold text-white mb-4">{GENERIC_SERVICE_DETAILS.aboutServiceTitle[language]}</h3>
                        <p className="leading-relaxed mb-8">
                            {GENERIC_SERVICE_DETAILS.intro[language]}
                        </p>
                            ...
                            <ul className="space-y-3 list-none pl-0 mb-0">
                                {GENERIC_SERVICE_DETAILS.benefits.map((benefit, i: number) => (
                                    <li key={i} className="flex items-start gap-3">
                                        ...
                                        <span>
                                            <strong className="text-white">{getLang(benefit.title, language)}:</strong>{' '}
                        ...
                        <h3 className="text-xl font-bold text-white mt-8 mb-4">{GENERIC_SERVICE_DETAILS.offerTitle[language]}</h3>
                ...
                {relatedPosts.length > 0 && (
                  <div className="border-t border-white/10 pt-12">
                      <h2 className="text-2xl font-bold text-white mb-8 flex items-center gap-3">
                        ...
                        İlgili İçgörüler
                        <span className="text-sm font-normal text-slate-400 ml-auto">Intelligence Hub</span>
                      </h2>
            ...
            <div className="lg:col-span-1">
                <div className="glass-card p-8 rounded-2xl sticky top-32">
                    <h3 className="text-xl font-bold mb-4">{GENERIC_SERVICE_DETAILS.ctaTitle[language]}</h3>
                    <p className="text-slate-400 mb-6 text-sm">
                        {GENERIC_SERVICE_DETAILS.ctaDesc[language]}
                    </p>
                    <Button className="w-full justify-center text-lg py-6" onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}>
