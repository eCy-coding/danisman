...
const apiKeyColumns: Column<ApiKeyItem>[] = [
  {
    ...
    label: 'Ad',
    sortable: true,
    render: (r) => <span className="text-white font-semibold">{r.name}</span>,
  },
  {
    key: 'scopes',
    label: 'Yetki',
    render: (r) => <code className="text-xs text-secondary">{r.scopes.join(', ')}</code>,
    hideOnMobile: true,
  },
  {
    key: 'lastUsedAt',
    label: 'Son Kullanım',
    hideOnMobile: true,
    render: (r) => (r.lastUsedAt ? new Date(r.lastUsedAt).toLocaleString('tr-TR') : '—'),
  },
  {
    key: 'revokedAt',
    label: 'Durum',
...
export const AdminSecurityPage: React.FC = () => {
  ...
  const keys = useQuery<ListResponse<ApiKeyItem>>({
    ...
    queryFn: () =>
      apiClient.get('/admin/security/api-keys').then((r) => r.data as ListResponse<ApiKeyItem>),
  });
  ...
  const logins = useQuery<ListResponse<LoginEvent>>({
    ...
    queryFn: () =>
      apiClient
        ...
        .then((r) => r.data as ListResponse<LoginEvent>),
  });
  ...
  const removeIp = useMutation({
    mutationFn: (ip: string) =>
      apiClient.delete(`/admin/security/ip-whitelist/${encodeURIComponent(ip)}`),
    onSuccess: () => {
    ...
    <div className="space-y-6">
      ...
      <header>
        ...
        <p className="text-sm text-slate-400 mt-1">
          API anahtarları, IP beyaz listesi, giriş geçmişi.
        </p>
      </header>
            ...
            content: (
              <div className="space-y-3">
                <p className="text-xs text-slate-500 inline-flex items-center gap-1.5">
                  <KeyRound size={12} /> Yeni anahtar üretimi auth.ts üzerinden (POST
                  /api/auth/api-keys). Buradan revoke edilir.
                </p>
                <DataTable
                  columns={[
                    ...apiKeyColumns,
          ...
          },
          {
            ...
            content: (
              <div className="space-y-4">
                <p className="text-xs text-slate-500 inline-flex items-center gap-1.5">
                  <ShieldCheck size={12} /> Admin paneline yalnızca bu IP'lerden erişim. Boşsa
                  kısıtlama yok.
                </p>
                <div className="flex gap-2 max-w-md">
                  <FormField label="" htmlFor="newip">
                    <input
                      ...
                      placeholder="örn. 185.45.12.0/24"
                ...
                <ul className="divide-y divide-white/5 bg-white/[0.02] border border-white/10 rounded-xl">
                  {(ips.data?.data?.items ?? []).length === 0 ? (
                    <li className="p-4 text-sm text-slate-400">
                      Beyaz liste boş (tüm IP'ler erişebilir).
                    </li>
                  ) : (
