...
const SERVICE_SLUGS = [
  'strategic-transformation', 'mergers-acquisitions', 'family-business',
  'operational-excellence', 'neuromarketing', 'hr-transformation',
  'crisis-management', 'ai-analytics', 'digital-strategy', 'data-governance',
  'esg-strategy', 'investment-incentives', 'macro-risk', 'competition-economics',
  'industrial-relations', 'payroll-audit', 'employer-branding', 'market-entry',
  'global-intelligence', 'smart-cities', 'government-relations',
];
...
export const AdminServiceEditPage: React.FC = () => {
  ...
  const query = useQuery<OverrideResponse>({
    ...
    queryFn: () => apiClient.get(`/admin/content/service/${slug}`).then((r) => r.data as OverrideResponse),
  ...
  if (!SERVICE_SLUGS.includes(slug)) {
    return (
      <div className="p-6">
        ...
        <Link to="/admin/services" className="text-secondary text-sm">← Servis listesi</Link>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <header className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => navigate('/admin/services')} aria-label="Servis listesine geri dön" className="text-slate-400 hover:text-white inline-flex items-center gap-1 text-sm">
      ...
      <Tabs items={[
        {
          ...
          content: (
            <div className="space-y-4 max-w-3xl">
              <FormField label="Hero Başlık" tooltip="Servis detay sayfasının ana başlığı (override).">
                <input type="text" value={data.heroTitle ?? ''} onChange={(e) => update('heroTitle', e.target.value)} className={fieldClassName} />
              </FormField>
              <FormField label="Hero Alt Başlık">
                <textarea value={data.heroSubtitle ?? ''} onChange={(e) => update('heroSubtitle', e.target.value)} className={`${fieldClassName} min-h-[80px]`} />
              </FormField>
              <FormField label="Value Proposition" tooltip="3-5 cümlelik değer önerisi.">
                <textarea value={data.valueProp ?? ''} onChange={(e) => update('valueProp', e.target.value)} className={`${fieldClassName} min-h-[100px]`} />
              </FormField>
            </div>
          ),
        },
          ...
          content: (
            <div className="space-y-4 max-w-3xl">
              <FormField label="Pain Points" hint="Her satıra bir madde">
                <textarea value={data.painPoints ?? ''} onChange={(e) => update('painPoints', e.target.value)} className={`${fieldClassName} min-h-[140px]`} />
              </FormField>
              <FormField label="Outcomes" hint="Her satıra bir sonuç">
                <textarea value={data.outcomes ?? ''} onChange={(e) => update('outcomes', e.target.value)} className={`${fieldClassName} min-h-[140px]`} />
              </FormField>
              <FormField label="Anonim Case Study" tooltip="NDA uyumlu özet (3-5 cümle).">
                <textarea value={data.caseStudyAnonymized ?? ''} onChange={(e) => update('caseStudyAnonymized', e.target.value)} className={`${fieldClassName} min-h-[100px]`} />
              </FormField>
            </div>
          ),
        },
          ...
          content: (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl">
              <FormField label="Yatırım Aralığı" tooltip="Örn: 250-650K TL / 8-14 hafta">
                <input type="text" value={data.investmentRange ?? ''} onChange={(e) => update('investmentRange', e.target.value)} className={fieldClassName} />
              </FormField>
              <FormField label="Timeline">
                <input type="text" value={data.timeline ?? ''} onChange={(e) => update('timeline', e.target.value)} className={fieldClassName} />
              </FormField>
            </div>
          ),
        },
