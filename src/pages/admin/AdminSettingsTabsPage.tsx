...
export const AdminSettingsTabsPage: React.FC = () => {
  ...
  const query = useQuery<SettingsResponse>({
    ...
    queryFn: () => apiClient.get('/admin/content/page/settings').then((r) => r.data as SettingsResponse),
  });
  ...
  const save = useMutation({
    mutationFn: (payload: SettingsShape) => apiClient.patch('/admin/content/page/settings', payload),
    onSuccess: () => {
  ...
  const upd = <K extends keyof SettingsShape>(k: K, v: SettingsShape[K]) => setData((d) => ({ ...d, [k]: v }));

  const f = (label: string, key: keyof SettingsShape, type: 'text' | 'textarea' | 'url' | 'color' = 'text', tooltip?: string) => (
    <FormField label={label} tooltip={tooltip}>
      {type === 'textarea' ? (
        <textarea value={(data[key] as string) ?? ''} onChange={(e) => upd(key, e.target.value as SettingsShape[typeof key])} className={`${fieldClassName} min-h-[100px]`} />
      ) : type === 'color' ? (
        <input type="color" value={(data[key] as string) ?? '#000000'} onChange={(e) => upd(key, e.target.value as SettingsShape[typeof key])} className="h-10 w-20 rounded border border-white/15 bg-neutral cursor-pointer" />
      ) : (
        <input type={type === 'url' ? 'url' : 'text'} value={(data[key] as string) ?? ''} onChange={(e) => upd(key, e.target.value as SettingsShape[typeof key])} className={fieldClassName} />
      )}
    </FormField>
  );
    ...
    <div className="space-y-6">
      ...
      <header className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Site, marka, entegrasyon, SEO, yasal, cookie, e-posta.</p>
        </div>
        <button
          type="button"
          ...
          content: (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl">
              ...
              <p className="md:col-span-2 text-xs text-slate-500">
                Bu alanlar canlı sistemde environment variable'larla override edilir. Burada kaydedilen
                değerler yalnızca dokümantasyon amaçlıdır.
              </p>
            </div>
          ),
        },
        ...
        },
        {
          ...
          content: (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl">
              ...
              <div className="md:col-span-2">{f('E-posta İmzası', 'emailSignature', 'textarea')}</div>
            </div>
          ),
        },
      ]} />
